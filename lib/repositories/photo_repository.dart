...
class PhotoRepository {
  ...
  Future<List<PhotoAlbum>> getAllAlbums() async {
    final res = await _db.from('photo_groups')
      .select('*, profiles(username, avatar_url), photo_images(id, url, caption, sort_order)')
      .order('created_at', ascending: false);
    return (res as List).map((j) => PhotoAlbum.fromJson(Map<String, dynamic>.from(j))).toList();
  }
  ...
  Future<PhotoAlbum> createAlbum({
    ...
    final group = await _db.from('photo_groups').insert({
      'title': title,
      'author_id': authorId,
      'thumbnail_url': thumbnailUrl,
    }).select().single();

    await _db.from('photo_images').insert(images.mapIndexed((i, img) => {
      'group_id': group['id'],
      'url': img['url'],
      'caption': img['caption'],
      'sort_order': i,
  ...
  Future<void> togglePublic(String id, bool current) async {
    await _db.from('photo_groups').update({'is_public': !current}).eq('id', id);
  }
}
