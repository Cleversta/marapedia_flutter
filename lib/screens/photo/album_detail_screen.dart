...
class _AlbumDetailScreenState extends State<AlbumDetailScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<PhotoBloc, PhotoState>(
      builder: (context, state) {
        if (state is PhotoLoading) return Scaffold(
  ...
  Widget _buildAlbum(PhotoAlbum album) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(album.title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
        ...
        Column(children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              Text(album.profile?['username'] ?? 'Anonymous', style: const TextStyle(fontSize: 13)),
              const Text(' · ', style: TextStyle(color: Colors.grey)),
              Text(Helpers.formatDate(album.createdAt), style: TextStyle(fontSize: 12, color: Colors.grey[500])),
              const Spacer(),
              Text('${images.length} photo${images.length != 1 ? 's' : ''}', style: TextStyle(fontSize: 12, color: Colors.grey[400])),
            ]),
          ),
          const Divider(height: 1),
          // Grid
          Expanded(child: images.isEmpty
            ? const Center(child: Text('No photos in this album.', style: TextStyle(color: Colors.grey)))
            : GridView.builder(
      ...
      child: Container(
        ...
        child: SafeArea(
          child: Stack(children: [
              ...
              child: GestureDetector(
                ...
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  if (images[_lightboxIdx!].caption != null) ...[
                    ...
                    Text(images[_lightboxIdx!].caption!, style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 13)),
                  ],
                  const SizedBox(height: 6),
                  Text('${_lightboxIdx! + 1} / ${images.length}',
            ...
            if (_lightboxIdx! > 0)
              Positioned(left: 4, top: 0, bottom: 0, child: Center(child: IconButton(
                onPressed: () => setState(() => _lightboxIdx = _lightboxIdx! - 1),
                icon: Container(
                  width: 40, height: 40,
                  decoration: BoxDecoration(color: Colors.white.withOpacity(0.15), shape: BoxShape.circle),
                ...
                children: images.asMap().entries.map((e) => GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: e.key == _lightboxIdx ? Colors.white : Colors.white30,
