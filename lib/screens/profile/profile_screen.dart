  ...
  void initState() {
    ...
    if (authState is AuthAuthenticated) {
      context.read<ArticleBloc>().add(
        ArticleMyListLoadRequested(authState.userId),
      );
      context.read<PhotoBloc>().add(
        PhotoMyAlbumsLoadRequested(authState.userId),
      );
    ...
    try {
      ...
      if (mounted) {
        context.read<AuthBloc>().add(
          AuthAvatarUpdateRequested(authState.userId, url),
        );
      }
    } catch (e) {
      if (mounted) {
                      ...
                      return BlocBuilder<PhotoBloc, PhotoState>(
                        builder: (context, photoState) {
                          final albums = photoState is PhotoMyAlbumsLoaded
                              ? photoState.albums
                              : <dynamic>[];
                          final totalPhotos = albums.fold(
                            0,
                            (s, a) => s + a.images.length as int,
                          );
                          return Row(
                            children: [
                              _statCard(
